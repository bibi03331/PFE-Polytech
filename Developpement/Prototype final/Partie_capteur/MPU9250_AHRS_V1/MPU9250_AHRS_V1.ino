...
void setup()
{
  ...
  if (c == 0x71) // WHO_AM_I should always be 0x68
  {
    ...
    Serial.print("x-axis accel bias : "); Serial.println(gyroBias[0]);
    Serial.print("y-axis accel bias : "); Serial.println(gyroBias[1]);
    Serial.print("z-axis accel bias : "); Serial.println(gyroBias[2]);
    Serial.print("x-axis gyro bias : "); Serial.println(accelBias[0]);
    Serial.print("y-axis gyro bias : "); Serial.println(accelBias[1]);
    Serial.print("z-axis gyro bias : "); Serial.println(accelBias[2]);
  ...
  magbias[0] = +19.;  // User environmental x-axis correction in milliGauss
  magbias[1] = +161.;  // User environmental y-axis correction in milliGauss
  magbias[2] = -170.;  // User environmental z-axis correction in milliGauss
...
void loop()
{
  ...
  if (readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01) {  // On interrupt, check if data ready interrupt
    ...
    ax = (float)accelCount[0] * aRes + 0.033; // - accelBias[0];  // get actual g value, this depends on scale being set
    ay = (float)accelCount[1] * aRes - 0.106; // - accelBias[1];
    az = (float)accelCount[2] * aRes + 0.030; // - accelBias[2];

    readGyroData(gyroCount);  // Read the x/y/z adc values
    // Calculate the gyro value into actual degrees per second
    gx = (float)gyroCount[0] * gRes + 0.5; // - gyroBias[0]; // get actual gyro value, this depends on scale being set
    gy = (float)gyroCount[1] * gRes - 1.0; // - gyroBias[1];
    gz = (float)gyroCount[2] * gRes + 1.0; // - gyroBias[2];
...
void calibrateMPU9250(float * dest1 /*gyroBias*/, float * dest2 /*accelBias*/)
{
  ...
  delay(100); // accumulate 40 samples in 40 milliseconds = 480 bytes
