...
void setup()
{
  ...
  if (c == 0x71) // WHO_AM_I should always be 0x68
  {
    ...
    byte d = readByte(AK8963_ADDRESS, WHO_AM_I_AK8963);  // Read WHO_AM_I register for AK8963. Should always be 0x48
    delay(1000);
...
void loop()
{
    ...
    if (readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01) {
      ...
      ax = (float)accelCount[0] * aRes - 0.024;
      ay = (float)accelCount[1] * aRes - 0.030;
      az = (float)accelCount[2] * aRes + 0.024;
  
      readGyroData(gyroCount);  // Read the x/y/z adc values
      // Calculate the gyro value into actual degrees per second
      gx = (float)gyroCount[0] * gRes + 0.3;
      gy = (float)gyroCount[1] * gRes - 1.0;
      gz = (float)gyroCount[2] * gRes + 1.0;
  
      readMagData(magCount);  // Read the x/y/z adc values
      // Calculate the magnetometer values in milliGauss
      // Include factory calibration per data sheet and user environmental corrections
      // 5 : 142 : -146
      mx = (float)magCount[0] - 5; // get actual magnetometer value, this depends on scale being set
      my = (float)magCount[1] - 142;
      mz = (float)magCount[2] + 146;
    }
